...
chrome.storage.local.get(["token", "trackingEnabled"], ({ token, trackingEnabled }) => {
...
function setStatus(type, title, meta = "") {
  const labels = { active: "Assistindo agora", paused: "Pausado", inactive: "Inativo", error: "Erro" };
...
toggle.addEventListener("change", () => {
  chrome.storage.local.set({ trackingEnabled: toggle.checked }, refreshStatus);
});

btnSave.addEventListener("click", () => {
  const value = tokenIn.value.trim();
  if (!value) { showHint("Cole o token antes de salvar.", "error"); return; }
  chrome.storage.local.set({ token: value }, () => showHint("Token salvo com sucesso! ✓", "ok"));
});
