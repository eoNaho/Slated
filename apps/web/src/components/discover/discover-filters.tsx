...
export function DiscoverFilters({ 
  ...
  const hasActiveFilters = filters.type !== "all" || filters.genre || filters.year || filters.streaming || filters.sortBy !== "popularity";
          ...
          >
            <div className="p-6 bg-zinc-900/30 rounded-3xl border border-white/5 backdrop-blur-sm space-y-8">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">Genres</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {/* Streaming Services */}
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">Streaming On</label>
                  <div className="flex flex-wrap gap-2">
                    {streamingServices.map((s) => (
                      <button
                        key={s.id}
                        onClick={() => updateFilter("streaming", filters.streaming === s.slug ? "" : s.slug)}
                        className={cn(
                          "w-10 h-10 rounded-xl border transition-all overflow-hidden flex items-center justify-center p-1.5 bg-zinc-950",
                          filters.streaming === s.slug 
                      ...
                      >
                        {s.logoPath ? (
                          <img src={s.logoPath} alt={s.name} className="w-full h-full object-contain rounded-[4px]" />
                        ) : (
                          <span className="text-[8px] font-bold">{s.name.slice(0, 3)}</span>
                        )}
                      </button>
                    ))}
                  </div>
                ...
                {/* Years */}
                <div className="space-y-3">
                   <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">Year</label>
                   <select 
                     value={filters.year}
                ...
                {/* Sort By */}
                <div className="space-y-3">
                   <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">Sort By</label>
                   <select 
                ...
                <div className="flex gap-2">
                  {hasActiveFilters && (
                    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/10 text-purple-400 text-[10px] font-bold uppercase tracking-wider">
                      {Object.values(filters).filter(v => v && v !== 'all' && v !== 'popularity').length} Active Filters
                    </div>
                  )}
                </div>
              </div>
