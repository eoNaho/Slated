...
import { slugify, resolveImage } from "@/lib/utils";
...
export function FavoriteFilms({ films, isEditable, customCovers = {} }: FavoriteFilmsProps) {
  return (
    <section>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {(films as FavoriteFilmWithDirector[]).map((film) => {
          const posterSrc = customCovers[film.id] 
            ? resolveImage(customCovers[film.id]) 
            : resolveImage(film.posterPath);

          return (
            <Link
              key={film.id}
              href={`/${film.mediaType === "series" ? "series" : "movies"}/${
