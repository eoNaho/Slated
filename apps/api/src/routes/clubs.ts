...
export const clubsRoutes = new Elysia({ prefix: "/clubs", tags: ["Clubs"] })
  ...
  .get(
    ...
    async ({ query }: any) => {
      ...
      conditions.push(eq(clubs.isPublic, true));

      if (category) {
        // Filter clubs that contain this category in their array
        conditions.push(
  ...
  .get(
    ...
    async (ctx: any) => {
      ...
      const [club] = await db
        ...
        .where(
          or(
            eq(clubs.id, params.id),
            eq(clubs.slug, params.id)
          )
        )
        .limit(1);
      ...
      if (!club.club.isPublic) {
        ...
        const [membership] = await db
          ...
          .where(
            and(
              eq(clubMembers.clubId, club.club.id),
              eq(clubMembers.userId, sessionUser.id)
            )
          )
          .limit(1);
        if (!membership) {
  ...
  .patch(
    ...
    async (ctx: any) => {
      ...
      const [membership] = await db
        ...
        .from(clubMembers)
        .where(
          and(eq(clubMembers.clubId, params.id), eq(clubMembers.userId, user.id))
        )
        .limit(1);
    ...
    async (ctx: any) => {
      ...
      if (!club.isPublic) {
        ...
        const [membership] = await db
          ...
          .from(clubMembers)
          .where(
            and(eq(clubMembers.clubId, params.id), eq(clubMembers.userId, sessionUser.id))
          )
          .limit(1);
        if (!membership) {
          set.status = 403;
  ...
  .post(
    ...
    async (ctx: any) => {
      ...
      const [alreadyMember] = await db
        ...
        .from(clubMembers)
        .where(
          and(eq(clubMembers.clubId, params.id), eq(clubMembers.userId, invitedUser.id))
        )
        .limit(1);
  ...
  .post(
    ...
    async (ctx: any) => {
      ...
      const [invite] = await db
        ...
        .from(clubInvites)
        .where(
          and(eq(clubInvites.id, params.inviteId), eq(clubInvites.invitedUserId, user.id))
        )
        .limit(1);
      ...
      if (new Date() > invite.expiresAt) {
        await db
          .update(clubInvites)
          .set({ status: "expired" })
          .where(eq(clubInvites.id, params.inviteId));
        set.status = 400;
