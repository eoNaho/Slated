...
const CURRENT_SCHEME: SchemeVersion =
  (process.env.ENCRYPTION_SCHEME as SchemeVersion) ?? "h1";
...
async function deriveKey(
  ...
  return scheme === "s1"
    ...
    : Promise.resolve(deriveKeyHkdf(conversationId));
}
...
export async function decrypt(
  ...
  const payload = colonIdx !== -1 ? data.slice(colonIdx + 1) : data;
  ...
  const encryptedBuf = buf.subarray(28);
