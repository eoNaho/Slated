 ...
 *    A separate `uploadSeasonStills(seasonId)` method can be called
 ...
 * Fires in the background after the initial `syncMedia` call.
 */
export async function deepSyncSeries(
  ...
  // 3. Update series season count (episodesCount is updated per-season)
  const { media: mediaTable } = await import("../db");
  await db
    .update(mediaTable)
    .set({
