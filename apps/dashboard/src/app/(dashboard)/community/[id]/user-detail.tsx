...
import { Users, Loader2, ArrowLeft, Clock, Monitor, Shield, Ban, CheckCircle, ShieldAlert, ChevronDown } from "lucide-react";
            ...
            <div className="glass-card rounded-2xl p-6 space-y-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-zinc-800 border border-white/10 overflow-hidden relative shrink-0">
                  {userDetail.avatarUrl ? (
                    <Image src={resolveImage(userDetail.avatarUrl) ?? ""} fill alt="" className="object-cover" />
                  ) : (
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {userDetail.isPremium && (
                  <span className="text-xs font-semibold px-2 py-0.5 rounded-full border text-amber-400 bg-amber-500/10 border-amber-500/20">Premium</span>
                )}
              </div>

              {userDetail.bio && <p className="text-sm text-zinc-400 leading-relaxed">{userDetail.bio}</p>}

              <div className="text-xs text-zinc-500 space-y-1">
                <p>Cadastrado em {formatDate(userDetail.createdAt)}</p>
                {userDetail.lastActiveAt && <p>Ativo em {formatDate(userDetail.lastActiveAt)}</p>}
              </div>
                  ...
                  {loginHistory.map((entry) => (
                    <div key={entry.id} className="flex items-center gap-4 px-6 py-3.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {entry.userAgent && (
                          <p className="text-xs text-zinc-600 truncate">{entry.userAgent}</p>
                        )}
                      </div>
                      <div className="text-right shrink-0">
                        {entry.deviceType && (
                          <span className="text-xs text-zinc-500 capitalize">{entry.deviceType}</span>
                        )}
